...
QJsonObject GeoGraphClass::createEllipseFeature(const QGeoCoordinate center, double semiMajorAxis, double semiMinorAxis, int segments)
{
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        ellipseCoordinates.append(QJsonArray{x, y});
    }
    // Close the ellipse by appending the first point again
    ellipseCoordinates.append(ellipseCoordinates.first());

    geometry["type"] = "Polygon";
    geometry["coordinates"] = QJsonArray{ellipseCoordinates};
    feature["geometry"] = geometry;
