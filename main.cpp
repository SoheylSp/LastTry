...
void rotateTestingFunct(){
    ...
    QList <QGeoCoordinate> myCoods ;

    myCoods.append(point1);
    myCoods.append(point2);

    QJsonObject lineFeature = GeoGraphClass::createLineFeature(myCoods);

    QGeoCoordinate lineCenter = ModifierClass::findCenterOfGeometry(lineFeature);

    QJsonObject rotatedFeature = ModifierClass::rotateModifier(lineCenter,45,lineFeature);

    QJsonArray orgCoordsArray  = UnitTest::getGeometryCoords(lineFeature);
    QJsonArray rotatedCoordsArray = UnitTest::getGeometryCoords(rotatedFeature);

    UnitTest::testOfRotation(orgCoordsArray,rotatedCoordsArray,lineCenter,45) ;
...
int main(int argc , char *argv[])
{
    ...
    if (myArguments.contains("rotate")) {
        if (!myArguments.contains("rotateAngle")) {
            qCritical() << "A rotateAngle is needed\n";
            return -1; // Exit the function if rotateAngle is missing
        }

        QGeoCoordinate rotatePoint;
        if (myArguments.contains("rotatePoint")) {
            ...
            if (rotatePointParts.length() == 2) {
                ...
                qCritical() << "Invalid rotatePoint format\n";
                return -1; // Exit the function if rotatePoint is invalid
            }
        } else {
            // If rotatePoint is not provided, find the center of the geometry
            rotatePoint = ModifierClass::findCenterOfGeometry(newFeature);
            qDebug() << "Center is: " << rotatePoint << "\n";
        }

        double rotateAngle = myArguments["rotateAngle"].toDouble();
        qDebug() << "RotateAngle is: " << rotateAngle << "\n";

        newFeature = ModifierClass::rotateModifier(rotatePoint, rotateAngle, newFeature);
        qDebug() << "New Feature is: " << newFeature << "\n";
