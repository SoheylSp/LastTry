...
int main(int argc, char *argv[])
{
    ...
    if (myArguments.contains("addLine")) {
        ...
        if (coordinateStrings.length() >= 2) {
            ...
            QJsonObject lineFeature = geoGraph.createLineFeature(geoCoordinates);
            QJsonArray featureArray;
            featureArray.append(lineFeature);
            geojsonIO.writeFeaturesToFile(featureArray,myFilePath, append);
        ...
        if (coordinateStrings.length() >= 3) { // A polygon requires at least 3 points
            ...
            QJsonObject polygonFeature = geoGraph.createPolygonFeature(geoCoordinates);
            QJsonArray featureArray;
        ...
        QJsonObject circleFeature = geoGraph.createCircleFearute(center, radius);
        QJsonArray featureArray;
        ...
        QJsonArray features;
              for (int i = 0; i <= numPoints; ++i) {
                  ...
                  QJsonObject feature = geoGraph.createGreatCircleFeature(interMediatePoint);
                  features.append(feature);
              }
              // Write the features to the file
              geojsonIO.writeFeaturesToFile(features, myFilePath, true);
        ...
        QJsonObject ellipseFeature = geoGraph.createEllipseObject(center, semiMajorAxis, semiMinorAxis, segments);
        QJsonArray featureArray;
        featureArray.append(ellipseFeature);
        geojsonIO.writeFeaturesToFile(featureArray, myFilePath, true); // Assuming we always append for ellipses


    }else if (myArguments.contains("rotate")) {

        if (!myArguments.contains("filePath") || !myArguments.contains("rotateAngle")) {
                qCritical() << "Missing filePath or rotateAngle argument";
                return -1;
            }

            myFilePath = myArguments["filePath"].toString();
            QVariantList rotateAngles = myArguments["rotateAngle"].toList();

            QJsonObject geojson = geojsonIO.readGeojsonFromFile(myFilePath);
            QJsonArray features = geojson["features"].toArray();

            QJsonArray rotatedFeatures;
            for (const QJsonValue& feature : features) {
                QJsonObject rotatedFeature = feature.toObject();
                for (const auto& angle : rotateAngles) {
                    rotatedFeature = ModifierClass::rotateModifier(angle.toDouble(), rotatedFeature);
                }
                rotatedFeatures.append(rotatedFeature);
            }

            geojson["features"] = rotatedFeatures;
            geojsonIO.writeGeojsonToFile(geojson, myFilePath);
}
}
            ...
            if (argument == "--help") {
                ...
                if (i + 1 < arguments.length()) {
                    ...
                    if (value.startsWith("--")) {
                        ...
                        if (key == "coordinates" || key == "rotateAngle") {
                            // If the key already exists, append the new value to the existing list
                            if (argumentsMap.contains(key)) {
                                QVariantList values = argumentsMap[key].toList();
                                values.append(value);
                                argumentsMap[key] = values;
                            } else {
