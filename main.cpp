...
QList<QGeoCoordinate> parseCoordinates(const QVariantMap& arguments){
    ...
    if (arguments.contains("coordinates")) {
        ...
        qDebug() << "coordinatesVar:" << coordinatesVar;

        if (coordinatesVar.type() == QVariant::String) {
            QString coordinateStr = coordinatesVar.toString();
            QStringList coordinatePairs = coordinateStr.split(" ");
            ...
            qDebug() << "coordinatesList:" << coordinatesList;
...
void findCenterTest(){
    ...
    QList<QGeoCoordinate> myCoords ;

    QGeoCoordinate point1 , point2 ;

    point1.setLatitude(1);
    point1.setLongitude(2);

    point2.setLatitude(3);
    point2.setLongitude(4);

    myCoords.append(point1);
    myCoords.append(point2);

    QJsonObject lineFeature = GeoGraphClass::createLineFeature(myCoords);
    QGeoCoordinate lineCenter = ModifierClass::findCenterOfGeometry(lineFeature);





    double centerLat = lineCenter.latitude();
    double centerLon = lineCenter.longitude();

    qDebug()<<"center lat is : " << centerLat << "\n";
    qDebug()<<"center lon is : " << centerLon << "\n";

}

void translateTestingFunc(){





    QJsonObject orgFeature ;
    QJsonObject orgGeometry;
    QJsonObject orgProperties;
    QJsonArray orgCoordinates;

    orgFeature["type"] = "Feature" ;
    orgGeometry["type"] = "LineString" ;
    orgFeature["properties"] = orgProperties ;

    QGeoCoordinate point1 , point2 ;

    point1.setLatitude(12.12);
    point1.setLongitude(21.21);

    point2.setLatitude(76.76);
    point2.setLongitude(67.67);
    ...
    QJsonArray orginalCoords = UnitTest::getGeometryCoords(lineFeature);

    QJsonObject translatedFeatrue = ModifierClass::translatorModifier(10,lineFeature);

    QJsonArray translatedCoords = UnitTest::getGeometryCoords(translatedFeatrue) ;

    UnitTest::testOfTranslate(orginalCoords,translatedCoords,10);



}

void rotateTestingFunct()
{
    ...
    QJsonArray lineCoordArray = UnitTest::getGeometryCoords(lineFeature);

    QJsonObject rotatedLineFeature = ModifierClass::rotateModifier2(lineCenter,45,lineFeature);

    QJsonArray rotatedLineCoordArray = UnitTest::getGeometryCoords(rotatedLineFeature);

    qDebug() <<  "Line Coords Array size is :" << lineCoordArray.size() << "\n" ;

    qDebug() <<  "Rotated Line Coords Array size is :" << rotatedLineCoordArray.size() << "\n" ;

    UnitTest::testOfRotation2(lineCoordArray,rotatedLineCoordArray,lineCenter,45) ;
...
void scaleTestingFunc(){
    ...
    QJsonObject lineFeature = GeoGraphClass::createLineFeature(myCoords);
    QGeoCoordinate lineCenter = ModifierClass::findCenterOfGeometry(lineFeature);
    QJsonArray lineCoordsArray = UnitTest::getGeometryCoords(lineFeature);

    QJsonObject scaledFeature = ModifierClass::scaleModifier(lineCenter,6,lineFeature);

    QJsonArray scaleCoordsArray = UnitTest::getGeometryCoords(scaledFeature);

    UnitTest::testOfScal(lineCoordsArray,scaleCoordsArray,lineCenter,6);
...
int main(int argc , char *argv[])
{
    ...
    if(myArguments.contains("coordTest")){

        if(myArguments.contains("coordinates")){

            QList<QGeoCoordinate> myCoordiantes = parseCoordinates(myArguments);
            qDebug()<<myCoordiantes;
            return -1;
        }
    ...
    if (myArguments.contains("rotate"))
    {

        if(myArguments.contains("rotatePoint") && myArguments.contains("rotateAngle"))
        {

            QGeoCoordinate rotatePoint ;

            QStringList rotatePointParts ;

            rotatePointParts = myArguments["rotatePoint"].toString().split(",");
            double rotatePointLat = rotatePointParts[1].toDouble();
            double rotatePointLon = rotatePointParts[0].toDouble();

            rotatePoint.setLatitude(rotatePointLat);
            rotatePoint.setLongitude(rotatePointLon);
        ...
        }else if (myArguments.contains("rotateAngle") && ! myArguments.contains("rotatePoint"))
        {

            QGeoCoordinate rotatePoint = ModifierClass::findCenterOfGeometry(newFeature);
            double rotateAngle = myArguments["rotateAngle"].toDouble();
    ...
    if (myArguments.contains("scale")) {
        ...
        if(myArguments.contains("scalePoint") && myArguments.contains("scaleNumber")){
            ...
            double scalePointLat = scalePointParts[0].toDouble();
            double scalePointLon = scalePointParts[1].toDouble();

            scalePoint.setLatitude(scalePointLat);
            scalePoint.setLongitude(scalePointLon);
        ...
        }else if (!myArguments.contains("scalePoint") && myArguments.contains("scaleNumber")) {
    ...
    //Write the updated featuress to the geojson file
...
void help(){
    ...
    std::cout <<"./executableName --filePath /path/to/output.geojson --addLine --coordinates \"32.3,12.3 76.5,54.5\"\n\n";
