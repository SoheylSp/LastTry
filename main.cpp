...
QList<QGeoCoordinate> parseCoordinates(const QVariantMap& arguments) {
    ...
    if (arguments.contains("coordinates")) {
        ...
        if (coordinatesVar.canConvert<QVariantList>()) {
            // The coordinates are provided as a list of lists
            ...
            for (const QVariant& coordinateVar : coordinatesList) {
                if (coordinateVar.canConvert<QVariantList>()) {
                    ...
                    if (coordinateParts.length() == 2) {
                        bool latOk, lonOk;
                        double lat = coordinateParts[1].toDouble(&latOk);
                        double lon = coordinateParts[0].toDouble(&lonOk);
                        if (latOk && lonOk) {
