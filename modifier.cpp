...
QJsonObject ModifierClass::scaleModifier(QGeoCoordinate scalePoint, double scaleNumber, QJsonObject feature)
{
    ...
    if(geometryType == "Point" ){
        ...
        for(int i = 0 ; i < coordinates.size() ; i++){
            ...
            QJsonArray coordinatesArray = coordinates[i].toArray();

            double latitude = coordinatesArray[1].toDouble();
            double longitude = coordinatesArray[0].toDouble();
            ...
            coordinatesArray[0] = scaledPoint.longitude();
            coordinatesArray[1]=scaledPoint.latitude();
            coordinates[i] = coordinatesArray;
